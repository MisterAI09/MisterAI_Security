        ...
        data.articles.forEach(article => {
            if(article.title && article.title !== "[Removed]") {
                ...
                <div class="bg-black/40 border border-cyan-500/20 p-6 rounded-xl hover:bg-cyan-500/10 transition-all flex flex-col h-full">
                    ...
                    <h4 class="font-bold text-sm mb-4 text-right">${article.title}</h4>
                    <a href="${article.url}" target="_blank" class="mt-auto text-center text-[9px] font-black text-black bg-cyan-400 py-2 rounded-lg uppercase hover:bg-white transition-all">Open_Link</a>
                </div>`;
            }
        });
    ...
    try {
        ...
        data.articles.slice(0, 8).forEach(article => {
            ...
            <div class="border-b border-white/5 pb-2 hover:bg-orange-500/5 transition-all p-1">
                <a href="${article.url}" target="_blank" class="text-gray-400 hover:text-orange-400 leading-snug block text-right">
                    ${article.title}
                </a>
            </div>`;
        });
