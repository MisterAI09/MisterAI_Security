...
async function generateMisterAISatire() {
    ...
    try {
        ...
        try {
            ...
            // فشل جلب الأخبار يبقى مخفياً عن الزائر
            console.log("Status: Background context utilized.");
        }
        ...
        const aiResponse = await fetch(aiUrl, {
            ...
            body: JSON.stringify({
                contents: [{
                    parts: [{
                        text: `أنت MisterAI، فيلسوف جزائري ساخر. اكتب مقالاً ساخراً وعميقاً بالدارجة الجزائرية عن: ${eventsContext}. لا تستخدم مقدمات تقنية ولا عناوين، ابدأ مباشرة بأسلوبك الخاص.`
                    }]
                }]
            })
        });
        ...
        responseArea.innerHTML = `
            <div class="cyber-card p-10 md:p-16 rounded-[4rem] bg-gradient-to-b from-[#0f172a] to-black border-t border-white/5 shadow-2xl animate-in fade-in duration-1000">
                ...
                <div class="mt-12 pt-8 border-t border-white/5 flex justify-between items-center">
                    ...
                    <p class="text-[8px] text-gray-700 uppercase tracking-widest">Verified_Intelligence</p>
                </div>
            </div>`;

    } catch (err) {
        // رسالة الخطأ "المستورة" فلسفياً
        console.error("Internal Log:", err); // يظهر للمبرمج فقط في الكونسول
        responseArea.innerHTML = `
...
/**
 * 2. جلب مختبر المعرفة (قسم البرمجة)
 */
async function fetchLabKnowledge() {
    ...
    try {
        ...
        console.log("Lab status: Offline");
    }
}
        ...
        if (data.articles) {
            container.innerHTML = data.articles.map(a => `
                <div class="border-b border-white/5 pb-3 mb-3">
                    <a href="${a.url}" target="_blank" class="text-gray-500 text-[11px] hover:text-white transition-all block leading-relaxed line-clamp-2">
                        ${a.title}
                    </a>
                </div>
            `).join('');
